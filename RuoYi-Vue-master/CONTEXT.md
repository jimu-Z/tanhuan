# 学生谈心谈话系统 — 领域术语

## 角色

- **辅导员（Counselor / talk_counselor）**：一线谈话执行者，负责大批量日常谈话发起与记录，关心"今天要跟谁谈、还有谁没谈"。**数据范围：仅自己管理的班级**（通过 `talk_teacher_class` 关联表按班级过滤学生和谈话记录）
- **书记（Secretary / talk_secretary）**：全局管理者，具有完整 CRUD 权限（含新增/修改/删除），关注统计数据、谈话覆盖率与学生管理。**数据范围：仅本学院及下属部门**（年级→班级，通过 `secretaryDeptId` 过滤）
- **管理员（Admin）**：系统运维角色，管理基础数据导入、系统配置，具有标签管理与系统配置的独占权限。**数据范围：全校全部**
- **学生（Student）**：谈话对象，通过独立学生端登录（学号+初始密码123456），可查看自己的谈话记录、填写反馈，收到新谈话通知。**数据范围：仅自己的记录**

### 角色菜单可见性矩阵

| 菜单 | 管理员 | 书记 | 辅导员 | 学生 | 备注 |
|------|:------:|:----:|:------:|:----:|------|
| 学生信息管理 (2001) | ✅ | ✅ | ✅ | ❌ | |
| 谈话管理 (2010) | ✅ | ✅ | ✅ | ❌ | |
| 发起谈话 (2030) | ✅ | ✅ | ✅ | ❌ | |
| 我的谈话记录 (2040) | ✅ | ✅ | ✅ | ❌ | |
| 统计分析 (2050) | ✅ | ✅ | ❌ | ❌ | |
| 预警提醒 (2055) | ✅ | ✅ | ✅ | ❌ | |
| 数据大屏 (2065) | ✅ | ✅ | ❌ | ❌ | hidden |
| 统一查询 (2070) | ✅ | ✅ | ✅ | ❌ | |
| 教师管理 (2090) | ✅ | ✅ | ❌ | ❌ | |
| 预约管理 (2100) | ✅ | ❌ | ❌ | ✅ | |
| 谈话模板库 (2060) | ✅ | ✅ | ✅ | ❌ | |
| 谈话跟进 (2075) | ✅ | ✅ | ✅ | ❌ | **hidden（菜单隐藏，功能正常）** |
| 标签管理 (2080) | ✅ | ❌ | ❌ | ❌ | |

## 数据权限架构

系统采用**双层过滤机制**确保角色数据隔离：

### 第一层：业务自定义 applyCounselorFilter() / applySecretaryFilter()

应用于以下 ServiceImpl 的列表查询方法：

| ServiceImpl | 辅导员过滤方式 | 书记过滤方式 |
|-------------|---------------|-------------|
| `TalkStudentServiceImpl` | `talk_teacher_class` 关联 → 按 classId 过滤学生 | `sys_dept.ancestors` 包含 secretaryDeptId |
| `TalkSessionServiceImpl` | `talk_session.create_by = counselorUsername` | 同上 |
| `TalkStudentRecordServiceImpl` | JOIN `talk_session.create_by` 过滤 | 同上 |
| `TalkAlertServiceImpl` | JOIN `talk_student.dept_id` 过滤 | 同上 |

### 第二层：RuoYi 框架 @DataScope AOP 切面

通过 `@DataScope(deptAlias = "d", userAlias = "u")` 注解自动注入 SQL WHERE 条件。适用于标准 CRUD 页面的 Mapper XML。

### 第三层：Controller 层防御性过滤（第五轮新增）

针对统计类接口的特殊处理：

| 接口 | 过滤逻辑 |
|------|---------|
| `TalkStatisticsController.dashboard()` — collegeRanking | 书记限本学院；辅导员隐藏该卡片；管理员全量 |
| `TalkStatisticsController.alerts()` — deptCoverage | 书记限本学院及下属部门；辅导员隐藏；管理员全量 |
| `TalkStudentServiceImpl.selectUntalkedStudentsInPeriod()` | 新增 params 参数支持 counselorUsername + secretaryDeptId 过滤 |

### 数据权限验证清单

```
[ ] 辅导员登录后只能看到自己管理的班级的学生
[ ] 辅导员只能看到自己创建的谈话会话
[ ] 辅导员不能看到其他辅导员的谈话内容
[ ] 书记只能看到本学院的数据（跨学院不可见）
[ ] 书记可以管理本学院的辅导员
[ ] 书记的统计分析只展示本学院排名
[ ] 管理员可以看到全校所有数据
[ ] 学生只能看到自己的谈话记录（已脱敏）
```

## 首页

- 首页按角色差异化展示：辅导员看到面向执行的工作台，书记看到面向统计与管理的仪表盘
- 管理员默认等同于书记视角
- 辅导员首页卡片：发起谈话、我的记录、学生管理、预警提醒、谈话模板库、谈话跟进
- 书记首页卡片：学生信息管理、发起谈话、统计分析、预警提醒、数据大屏、统一查询
- 学生首页：我的谈话记录、待处理谈话、个人中心
- 统计分析仪表盘（去竞争化）：工作提醒（待处理反馈/待跟进/未谈话学生）+ 预警概览（心理健康/贫困/学籍异常）+ 最近谈话动态

## 生产部署

- 服务器：211.64.39.248
- 端口分配：后端 6060 / 管理端前端 6080 / 学生端前端 6090
- Nginx 反向代理：6080/6090 站点的 `/prod-api/` 代理到后端 6060
- 静态文件路径：管理端 `/www/wwwroot/talk/deploy/admin-ui/`，学生端待部署
- 宝塔面板管理：Java 项目管理器运行后端 JAR，HTML 项目管理前端站点
- Redis 配置：bind 127.0.0.1 211.64.39.248，protected-mode no

## 班级层级

- 班级通过 `sys_dept` 树表示，层级为 **学院 → 年级 → 班级**，学生挂在班级叶子节点下
- 选择学生时通过班级树导航，支持按班级批量选择（一键全选某班学生参加集体谈话）

## 谈话附件

- 一次谈话可附带多个附件文件（图片、PDF、Word 等），作为独立附件上传区，不嵌入谈话内容
- 附件表单独存储（`talk_attachment`），不限制单个文件大小

## 谈话标签

- 标签为系统全局定义，存储于 `talk_tag` 表，由管理员独占管理（新增、编辑、停用、逻辑删除）
- 停用或逻辑删除的标签不再出现在新建谈话的选择列表中，但历史已使用的标签数据保留不变
- 现有硬编码常量（`TalkConstants.TAG_LABELS`）将被替换为数据库驱动的标签读取

## 谈话通知

- **学生通知**：老师发起/编辑谈话后，学生端看到待处理谈话 + 顶栏红点通知
- **教师通知**：学生提交反馈后，教师端仪表盘显示"待查看反馈"数量
- 通知机制基于 `talk_student_record` 的标记字段简化实现（`notified` / `teacher_notified`），不建立独立通知表

## 学生端

- 学生端为独立 Vue2 + Element UI 前端项目，复用后端 API
- 学生账号在 Excel 导入时自动创建（登录名=学号，初始密码=123456），所有学生均可登录
- 学生端功能：我的谈话记录、待处理谈话（在线填写反馈）、个人中心（修改密码）
- 学生不能查看同次集体谈话中其他参与者的信息

## 集体谈话

- 集体谈话参与学生信息在三个位置展示：会话管理列表（缩略）、会话详情页（完整）、docx 导出（汇总表）
- docx 导出分为两种：每人一份独立记录（保留），集体谈话汇总表（新增，模板后续提供）
- 导出时附件不嵌入 docx

## 未谈学生筛选

- 在"统一查询"页面新增查询模式切换：查已有记录 / 查未谈学生
- 查未谈学生：选择班级 + 日期范围，列出该时间段内无任何谈话记录的学生名单
- 支持按班级维度快速查看已谈/未谈覆盖率
